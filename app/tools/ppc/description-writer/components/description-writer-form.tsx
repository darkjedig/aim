...
interface DescriptionWriterFormProps {
...
export function DescriptionWriterForm({ onSubmit, isLoading, selectedPlatform, setSelectedPlatform }: DescriptionWriterFormProps) {
  return (
    <motion.div 
    ...
    >
      <motion.h2 
        ...
        transition={{ duration: 0.5 }}
      >
        Generate Your PPC Descriptions
      </motion.h2>
      <motion.form 
        onSubmit={onSubmit} 
        className="space-y-6"
      ...
      >
        ...
        <Button type="submit" className="w-full bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 text-white">
          ...
          {isLoading ? 'Generating Descriptions...' : 'Generate Descriptions'}
        </Button>
      </motion.form>
    </motion.div>
  )
